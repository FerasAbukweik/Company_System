...
public class AccountOrgHierarchyService(
    ...
    ILogger<AccountOrgHierarchyService> logger
    ) : IAccountOrgHierarchyService
{
    public async Task<Result<ApplicationUser>> AddEmployee(AddEmployeeDTO toCreateData, CancellationToken cancellationToken = default)
    {
        ...
        logger.LogInformation("{serviceName}.{methodName} user with id {userId} was created and added to org tree with id {orgTreeId}",
            nameof(AccountOrgHierarchyService), nameof(AddEmployee), addUserToHierarchyResult.Value!.UserId, addUserToHierarchyResult.Value.Id );
