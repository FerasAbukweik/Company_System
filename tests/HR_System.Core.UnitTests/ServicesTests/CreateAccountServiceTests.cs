...
using HR_System.Core.Services.AccountServices;
...
public class CreateAccountServiceTests
{
    ...
    private readonly Mock<IAddCookieService> _addCookieServiceMock;

    public CreateAccountServiceTests(ITestOutputHelper output)
    {
        _output = output;
        ...
        _addCookieServiceMock = new Mock<IAddCookieService>();

        _createAccountService = new CreateAccountService(_userManagerMock.Object,
            _userRepositoryMock.Object,
            _roleManagerMock.Object,
            _addCookieServiceMock.Object,
            NullLogger<CreateAccountService>.Instance
        );
    }
    ...
    public async Task CreateAccountAsync_ValidData_ShouldSuccess()
    {
        ...
        _userRepositoryMock.Setup(t => t.FilterAsync(It.IsAny<Expression<Func<ApplicationUser, bool>>>()))
            .ReturnsAsync(Result<ApplicationUser[]>.Success([]));
        ...
        _addCookieServiceMock.Setup(t => t.AddTokensToCookies())
            .ReturnsAsync(Result<AccessAndRefreshTokenDTO>.Success(_fixture.Create<AccessAndRefreshTokenDTO>()));
        
        _output.WriteLine($"Expected:\n{toCreate.ToString()}");

        // Act
        var actual = await _createAccountService.CreateAccountAsync(toCreate);
    ...
    public async Task CreateAccountAsync_WhenUserAlreadyExists_ShouldReturnFailure()
    {
        ...
        _userRepositoryMock.Setup(t => t.FilterAsync(It.IsAny<Expression<Func<ApplicationUser, bool>>>()))
            .ReturnsAsync(Result<ApplicationUser[]>.Success([existingUser]));
